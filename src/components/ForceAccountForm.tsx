                ...
                <div className="p-8 pb-32">
                    ...
                    {editTab === "general" && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-1"><label className="text-[10px] uppercase font-black">Proyecto # (Manual)</label><input className="input-field" value={currentFA.admin || ""} onChange={e => setCurrentFA({...currentFA, admin: e.target.value})} placeholder="Para forzar en reporte"/></div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl border p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-xs">
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">a) Subtot Operadores (Unión)</span> $<input className="w-24 text-right bg-white border outline-none" type="number" value={currentFA.fa_details.mo_operadores} onChange={e=>updateDetail('mo_operadores', +e.target.value)}/></div>
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">b) Beneficios Oper. <input className="w-12 text-center border mr-1 text-xs" value={currentFA.fa_details.mo_operadores_pct} onChange={e=>updateDetail('mo_operadores_pct', +e.target.value)}/>%</span> <b>{formatCurrency(currentFA.fa_details.mo_operadores * (currentFA.fa_details.mo_operadores_pct/100))}</b></div>
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">c) Subtot Carpinteros</span> $<input className="w-24 text-right bg-white border outline-none" type="number" value={currentFA.fa_details.mo_carpinteros} onChange={e=>updateDetail('mo_carpinteros', +e.target.value)}/></div>
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">d) Beneficios Carp. <input className="w-12 text-center border mr-1 text-xs" value={currentFA.fa_details.mo_carpinteros_pct} onChange={e=>updateDetail('mo_carpinteros_pct', +e.target.value)}/>%</span> <b>{formatCurrency(currentFA.fa_details.mo_carpinteros * (currentFA.fa_details.mo_carpinteros_pct/100))}</b></div>
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">e) Subtot Adicional</span> $<input className="w-24 text-right bg-white border outline-none" type="number" value={currentFA.fa_details.mo_adicional} onChange={e=>updateDetail('mo_adicional', +e.target.value)}/></div>
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">f) Beneficios Adic. <input className="w-12 text-center border mr-1 text-xs" value={currentFA.fa_details.mo_adicional_pct} onChange={e=>updateDetail('mo_adicional_pct', +e.target.value)}/>%</span> <b>{formatCurrency(currentFA.fa_details.mo_adicional * (currentFA.fa_details.mo_adicional_pct/100))}</b></div>
                                    ...
                                    <div className="flex items-center justify-between py-2 border-b"><span className="w-48">l) Beneficios sin Unión <input className="w-12 text-center border mr-1 text-xs" value={currentFA.fa_details.mo_sin_union_pct} onChange={e=>updateDetail('mo_sin_union_pct', +e.target.value)}/>%</span>  <b>{formatCurrency(h_subtotal * (currentFA.fa_details.mo_sin_union_pct/100))}</b></div>
                                    
                                    <div className="col-span-1 md:col-span-2 mt-4 space-y-2">
                                        <div className="flex justify-between items-center bg-white p-2 border"><span className="w-48 font-bold">j) Gastos Viaje/Dietas</span> $<input className="w-32 text-right bg-slate-50 border p-1" type="number" value={currentFA.fa_details.mo_gastos_viaje} onChange={e=>updateDetail('mo_gastos_viaje', +e.target.value)}/></div>
                                        ...
                                        <div className="flex justify-between items-center p-2"><span className="w-64 font-bold">l) Beneficio Industrial <input className="w-12 text-center border" value={currentFA.fa_details.mo_beneficio_ind_pct} onChange={e=>updateDetail('mo_beneficio_ind_pct', +e.target.value)}/>% de k</span> <b>{formatCurrency(k * (currentFA.fa_details.mo_beneficio_ind_pct/100))}</b></div>
                                        ...
                                        <div className="flex justify-between py-1 pt-4 text-[11px]"><span className="w-64">n) Fondo Seguro Estado <input className="w-10 border text-center" value={currentFA.fa_details.mo_fondo_estado_pct} onChange={e=>updateDetail('mo_fondo_estado_pct', +e.target.value)}/>%</span> <b>{formatCurrency(h_subtotal * (currentFA.fa_details.mo_fondo_estado_pct/100))}</b></div>
                                        <div className="flex justify-between py-1 text-[11px]"><span className="w-64">o) Seguro Social <input className="w-10 border text-center" value={currentFA.fa_details.mo_seguro_social_pct} onChange={e=>updateDetail('mo_seguro_social_pct', +e.target.value)}/>%</span> <b>{formatCurrency(h_subtotal * (currentFA.fa_details.mo_seguro_social_pct/100))}</b></div>
                                        <div className="flex justify-between py-1 text-[11px]"><span className="w-64">p) Desempleo Est. <input className="w-8 border" value={currentFA.fa_details.mo_desempleo_est_pct} onChange={e=>updateDetail('mo_desempleo_est_pct', +e.target.value)}/>% + Fed. <input className="w-8 border" value={currentFA.fa_details.mo_desempleo_fed_pct} onChange={e=>updateDetail('mo_desempleo_fed_pct', +e.target.value)}/>%</span> <b>{formatCurrency(h_subtotal * ((currentFA.fa_details.mo_desempleo_est_pct + currentFA.fa_details.mo_desempleo_fed_pct)/100))}</b></div>
                                        <div className="flex justify-between py-1 text-[11px]"><span className="w-64">q) Seg. Resp. Pública <input className="w-10 border text-center" value={currentFA.fa_details.mo_resp_publica_pct} onChange={e=>updateDetail('mo_resp_publica_pct', +e.target.value)}/>%</span> <b>{formatCurrency(h_subtotal * (currentFA.fa_details.mo_resp_publica_pct/100))}</b></div>
                                        <div className="flex justify-between py-1 text-[11px] border-b"><span className="w-64">r) Incapacidad <input className="w-10 border text-center" value={currentFA.fa_details.mo_incapacidad_pct} onChange={e=>updateDetail('mo_incapacidad_pct', +e.target.value)}/>%</span> <b>{formatCurrency(h_subtotal * (currentFA.fa_details.mo_incapacidad_pct/100))}</b></div>
