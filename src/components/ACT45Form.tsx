...
const ACT45Form = forwardRef<FormRef, { projectId?: string; numAct?: string; onDirty?: () => void; onSaved?: () => void; }>(
  function ACT45Form({ projectId, numAct, onDirty, onSaved }, ref) {
    ...
    const load = async () => {
      const { data } = await supabase.from("projects").select("name, contract_number, municipality, act45_last_report").eq("id", projectId!).single();
      if (data) setD(p => ({ ...p, ...(data.act45_last_report || {}), numProyecto: numAct || p.numProyecto, nombreProyecto: data.name || p.nombreProyecto, municipio: data.municipality || p.municipio, fecha: new Date().toISOString().split("T")[0] }));
    };
