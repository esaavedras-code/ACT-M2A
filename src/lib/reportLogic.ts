...
import { generateSolicitudMaterialCertPdf } from "./generateSolicitudMaterialCertPdf";
...
export const generateSolicitudMaterialCertPdfLogic = async (projectId: string) => {
    const blob = await generateSolicitudMaterialCertPdf(projectId);
    const { data: project } = await supabase.from('projects').select('num_act').eq('id', projectId).single();
    if (blob) {
        await downloadBlob(blob, `Solicitud_Material_Certification_${project?.num_act || projectId}.pdf`);
    }
};
