import { LaborEntry, EquipmentEntry, MaterialEntry } from '../types';

export const calculateLaborTotal = (entry: LaborEntry) => {
  const regTotal = entry.hoursReg * entry.hourlyRate;
  const ot15Total = entry.hours15 * entry.hourlyRate * 1.5;
  const ot20Total = entry.hours20 * entry.hourlyRate * 2.0;
...
export const applyAC51Rules = (laborTotal: number, equipmentTotal: number, materialTotal: number) => {
  ...
  // 2. Equipo
  const benefitIndustrialEQ = equipmentTotal * 0.15; // 15% BI Equipo
  const finalEQTotal = equipmentTotal + benefitIndustrialEQ;

  // 3. Materiales
  const benefitIndustrialMAT = materialTotal * 0.15; // 15% BI Materiales
  const finalMATTotal = materialTotal + benefitIndustrialMAT;
